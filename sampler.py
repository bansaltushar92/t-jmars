            ...
            for movie in range(self.M):    
                for (rev,r) in movie_reviews[movie]:
                    ...
                    for i, word in enumerate(rev.strip().split()):
                        ...
                        self.topics[(r, i)] = (y, z, s)
                save_npz('./test/movie_aspects/%s'%(movie), self.cmyw[movie])

            print (self.Nums[:10,0], self.Numas[:10,0,0], self.Numa[:10,0], self.cy, self.cyw[:,16])
            np.save('./test/cyw.npy', self.cyw)
            np.save('./test/cyzw.npy', self.cyzw)
            np.save('./test/cysw.npy', self.cysw)
            np.save('./test/Nums.npy', self.Nums)
            np.save('./test/Numas.npy', self.Numas)
            np.save('./test/Numa.npy', self.Numa)
